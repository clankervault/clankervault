    ...
    if (opts.all) {
      ...
      for (const p of listProjects(dir)) {
        ...
        for (const name of tools) {
          ...
          if (!opts.force && existsSync(target) && !readFileSync(target, 'utf8').slice(0, 200).includes('GENERATED by vault')) {
      ...
      console.log(`compiled ${compiledProjects} projects, skipped ${skippedFiles} files`);
      logAccess(dir, 'compile', { all: true, projects: compiledProjects });
      return;
    }

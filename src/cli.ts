...
import { adapters, getAdapter, isGenerated } from './adapters/index.js';
...
program
  ...
  .action((dir?: string) => {
    ...
    console.log('Next: `clanker project new <name>` and edit me/profile.md');
  });
...
program
  ...
  .action((type: string, title: string, opts: { project?: string; body?: string; scope?: string; tags?: string; expires?: string }) => {
    if (!['fact', 'recipe', 'decision', 'taste'].includes(type)) {
      console.error(`Unknown type "${type}". Use: fact | recipe | decision | taste (work in progress goes to \`clanker state\`).`);
      process.exit(1);
    }
  ...
  .action((opts: { project?: string; all?: boolean; tool: string; out?: string; budget?: string; force?: boolean }) => {
    ...
    for (const name of tools) {
      ...
      if (!opts.force && existsSync(target) && !isGenerated(readFileSync(target, 'utf8').slice(0, 200))) {
        console.log(`skipped ${target}: existing file is not vault-generated (use --force to overwrite)`);
        continue;
      }
...
program
  ...
  .option('--from-now', 'skip everything already in the transcripts: fast-forward offsets to the current end of each file without extracting (onboarding; avoids paying to mine months of history)')
  .description('mine session transcripts into unconfirmed records (claude-code reader)')
  .action(async (opts: { root?: string; dryRun?: boolean; watch?: boolean; interval: string; fromNow?: boolean }) => {
    try {
      const dir = requireVault(vaultDir());
      if (opts.fromNow) {
        const root = opts.root ?? defaultTranscriptRoot();
        const offsets = readOffsets(dir);
        let forwarded = 0;
        for (const file of discoverTranscripts(root)) {
          const size = statSync(file).size;
          if ((offsets[file] ?? 0) < size) { offsets[file] = size; forwarded++; }
        }
        writeOffsets(dir, offsets);
        console.log(`fast-forwarded ${forwarded} transcripts; future sessions will be mined from here on`);
        return;
      }
      let intervalMs = 0;
      if (opts.watch) {
      ...
      const runOnce = async () => {
        ...
        if (r.created.length && !opts.dryRun) console.log('Review them with `clanker list`, release with `clanker confirm <id>` or let `clanker settle` confirm them after they age.');
      };
      await runOnce();
      if (opts.watch) {
...
sync
  ...
  .option('--url <u>', 'clankervault server URL (self-hosted sync server, see `clanker serve`)')
  ...
  .action((opts: { path?: string; url?: string; token?: string; passphrase?: string }) => {
    ...
    if (!raw.sync.passphrase) console.log('No passphrase saved: set VAULT_PASSPHRASE before running `clanker sync`.');
  });
sync
  ...
  .action(async (opts: { watch?: boolean; interval: string }) => {
    ...
    try {
      ...
      if (!device.sync) {
        console.error('Sync is not configured on this device. Run: clanker sync setup --path <remoteDir> (or --url <server> --token <t>)');
        process.exit(1);
      }
      const passphrase = process.env.VAULT_PASSPHRASE ?? device.sync.passphrase;
      if (!passphrase) {
        console.error('No passphrase. Set VAULT_PASSPHRASE or run `clanker sync setup` with --passphrase.');
        process.exit(1);
      }
...
program
  ...
  .description('run the self-hostable clankervault server (encrypted sync remote + remote MCP)')
  .action((opts: { data: string; port: string; token?: string }) => {
    try {
      ...
      server.listen(port, () => {
        ...
        console.log(`clankervault server listening on :${real} (sync API ready)`);

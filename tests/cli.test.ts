...
describe('vault CLI end to end', () => {
  ...
  it('never overwrites a hand-written CLAUDE.md, unless forced', () => {
    ...
    expect(readFileSync(join(work, 'CLAUDE.md'), 'utf8')).toContain('GENERATED by clankervault');
  ...
  it('still overwrites a file carrying the pre-rename "GENERATED by vault" header without --force', () => {
    const vault = join(tmpDir(), 'v');
    const work = tmpDir();
    run(['init', vault]);
    run(['--vault', vault, 'project', 'new', 'Demo', '--root', work]);
    ...
    const r = run(['--vault', vault, 'compile', '--tool', 'claude'], work);
    expect(r.code).toBe(0);
    expect(r.out).toMatch(/wrote .*CLAUDE\.md/);
    expect(readFileSync(join(work, 'CLAUDE.md'), 'utf8')).toContain('GENERATED by clankervault');
  });
  ...
  it('rejects unknown record type with guidance', () => {
    ...
    expect(r.out).toMatch(/clanker state/);
  });
